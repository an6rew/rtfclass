...
/*
  ...
  10.2015 an6rew
  Added cyrrilic support.
  Fixed fonttable loading.
  Fixed text alignment.
  Fixed new line with <br> for realy empty lines.
  Change space to &nbsp; in text for browser good rendering.
...
class rtf {
    ...
    function parseControl($control, $parameter) {
        switch ($control) {
            ...
            case "par":
                /* added by an6rew Empty paragraph after paragraph need new line <br /> */
                if ($this->wantHTML) {
    ...
    function flushGroup($state) {
        ...
        if ($state == "close") {
            ...
            $this->groupLevel--;
            if ($this->last_flags["fonttbl"] == true && $this->groupLevel == 1) { // by an6rew - fixed group loading
    ...
    function flushQueue() {
        if (strlen($this->queue)) {
            // processing logic
            /* fix by an6rew */
            if (isset($this->flags["fonttbl_current_write"]) &&
                    preg_match("/^[0-9]+$/", $this->flags["fonttbl_current_write"])) {
                if (!isset($this->fonttable[$this->flags["fonttbl_current_write"]])) {
                    $this->fonttable[$this->flags["fonttbl_current_write"]]["charset"] = $this->queue;
                }
                $this->queue = "";
            }
            /*
            if (isset($this->flags["fonttbl_want_fcharset"]) && // by an6rew
            ...
            */
            ...
            if (strlen($this->queue)) {
                ...
                if ($this->wantHTML) {
                    ...
                    if (isset($this->flags["fonttbl_current_read"]) && // by an6rew
                            preg_match("/^[0-9]+$/", $this->flags["fonttbl_current_read"])) {
                        if ($this->flags["beginparagraph"] == true) {
                        ...
                        $ind0 = "f" . $this->flags["fonttbl_current_read"] . "s" . $this->flags["fontsize"]; // by an6rew
                        $ind1 = $this->flags["fonttbl_current_read"]; // by an6rew
                        if (isset($this->fonttable[$ind1])) { // by an6rew
                            $this->styles[$ind0] = "font-family:" . $this->fonttable[$ind1]["charset"] . " font-size:" . $this->flags["fontsize"] . ";";
                        }
                        /* write span start */
    ...
    function flushSpecial($special) {
        if (strlen($special) == 2) {
            if ($this->wantXML) {
                ...
                $this->out .= "<special value=\"".$special."\"/>";
                switch ($special) { // by an6rew added switch special characters
    ...
    function makeStyles() {
        ...
        $this->out = $this->outstyles.$this->out; // by an6rew, realy added style to the document
    }
        ...
        while ($i < $this->len) {
            ...
            switch ($symbol) {
                ...
                case "\\":
                    ...
                    break;
                default:
                    if ((ord($symbol) == 10) || (ord($symbol) == 13)) {
                    ...
                    if ($this->cw) { // active control word ?
                         ...
                         */
                        if (preg_match("/^[a-zA-Z0-9-]?$/", $symbol)) { // continue parsing
                            $this->cword .= $symbol;
                            $this->cfirst = false;
                        } else {
                            /*
                            ...
                            $specialmatch = false;
                            if ($this->cfirst) {
                                if ($symbol == '\'') { // expect to get some special chars
                                    $this->flushQueue();
                                ...
                                if (preg_match("/^[{}\*]$/", $symbol)) {
                                    $this->flushComment($symbol." - control symbols not yet handled");
                                    $specialmatch = true;
                                }
                                $this->cfirst = false;
                            } else {
                                if ($symbol == ' ') { // space delimtes control words, so just discard it and flush the controlword
                                    $this->cw = false;
                                    $this->flushControl();
                                    break;
                                }
                        ...
                        if (($this->wantHTML) || ($this->wantXML)) {
                            switch ($symbol) {
                                ...
                                case ">":
                                    $this->queue .= "&gt;";
                                    break;
                                case " ": // added by an6rew Space to &nbsp; for good rendreing more spaces in browser
                                    if ($this->wantHTML &&
                                            isset($this->flags["fonttbl_current_read"]) &&
                                            preg_match("/^[0-9]+$/", $this->flags["fonttbl_current_read"])) {
                                        $this->queue .= "&nbsp;";
                                    } else {
                                        $this->queue .= $symbol;
    ...
    /**
     * (by an6rew) Replace cyrrilic symbol codes to symbol
     */
    function rtfregexp($data) {
        $pattern = array(
